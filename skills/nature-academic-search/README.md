...
- **严格他引与高影响力引用者审计**：判断引用目标论文的文献是否属于严格他引，排除自引、团队引和明显合作网络引用；进一步识别院士、校长/院长、杰青/长江、Fellow、高被引学者等高影响力引用者，并提取他们在正文中如何引用目标论文。
...
当用户询问 `严格他引`、`他引判定`、`谁引用了我的文章`、`引用我的文章的人有没有大牛`、`院士引用`、`杰青引用`、`长江学者引用` 或 `Fellow citation` 时，本技能会加载
...
4. 对严格他引或可能外部引用者，核验是否存在院士、校长/院长、杰青/长江、Fellow、高被引学者等身份信号。
5. 从可获取全文中抽取 citation context，判断对方是把目标论文作为背景、方法、对比、延伸、复现、批评还是综述性引用。
